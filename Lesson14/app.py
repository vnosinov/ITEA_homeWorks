...
DB_URL = 'postgresql://postgres:dbpass@localhost:5432/order_service_db'
...
class Order(db.Model):
    ...
    def __str__(self):
        return f"Order_id: {self.order_id}\n" \
               ...
               f"Serial_no: {self.serial_number}\n" \
...
def insert_db():
    if request.method == 'POST':
        ...
        order_profile = Order(order_type=data_order['type_order'],
                              description=data_order['description'],
                              status=data_order['status'],
                              serial_number=data_order['serial_number'],
